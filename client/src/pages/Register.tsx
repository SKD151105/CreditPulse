...
export const Register = () => {
  ...
  const handleDemoLogin = async (credentials: any) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.message || 'Failed to login as demo user');
      } else {
  ...
  return (
    <div className="min-h-screen pt-16 pb-4 flex items-center justify-center bg-[#101325] relative overflow-x-hidden">
      <motion.div 
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.5 }}
        className="absolute w-[500px] h-[500px] bg-purple-500/20 rounded-full blur-[100px] -top-20 right-0"
      />
      <motion.div 
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1.5, delay: 0.2 }}
        className="absolute w-[400px] h-[400px] bg-indigo-500/20 rounded-full blur-[100px] bottom-0 left-0"
      />
      ...
      >
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-8 text-white">Create Account</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white rounded-lg font-medium shadow-lg hover:shadow-indigo-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-6"
          >
        ...
        <div className="mt-8 pt-6 border-t border-gray-800">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={() => handleDemoLogin({ email: 'applicant1@demo.com', password: 'password123' })}
              disabled={isSubmitting}
              type="button"
